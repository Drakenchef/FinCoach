...
func (r *Repository) PlanetsRequestsList() (*[]models.PlanetsRequest, error) {
	var planetsRequest []models.PlanetsRequest
	result := r.db.Select("fr_id", "planet_id", "flight_number").Find(&planetsRequest)
	return &planetsRequest, result.Error
}

//var oldPlanetRequest models.PlanetsRequest
...
//	err := r.db.Model(&models.PlanetsRequest{}).Where("fr_id = ? AND flight_number = ?", updatedPlanetRequest.FRID, PlanetsRequest1.FlightNumber+1).Error
//	if err == nil {
//		r.db.Model(&models.PlanetsRequest{}).Where("fr_id = ? AND flight_number = ?", updatedPlanetRequest.FRID, PlanetsRequest1.FlightNumber+1).Update("flight_number", PlanetsRequest1.FlightNumber-1)
//	}
//	r.db.Model(&models.PlanetsRequest{}).Where("fr_id = ? AND planet_id = ?", updatedPlanetRequest.FRID, updatedPlanetRequest.PlanetID).Update("flight_number", PlanetsRequest1.FlightNumber+1)
//}
...
func (r *Repository) AddPlanetToRequest(pr *struct {
	...
	if FlightRequest.ID == 0 {
		newRequest := models.FlightRequest{UserID: user.ID, UserLogin: user.Login, Status: "создан", DateCreate: time.Now(), Result: "Отсутствует"}
	...
	flightNumber := 0
	r.db.Model(&models.PlanetsRequest{}).Where("fr_id = ?", FlightRequest.ID).Select("MAX(flight_number)").Scan(&flightNumber)
	query := "INSERT INTO planets_requests (fr_id, planet_id, flight_number) VALUES ($1,$2, $3) ON CONFLICT DO NOTHING;"
...
func (r *Repository) DeletePlanetRequest(frid, planetid uint) error {
	...
	err = r.db.Model(&models.PlanetsRequest{}).Where("fr_id = ? AND flight_number > ?", frid, planetsrequest.FlightNumber).Update("flight_number", gorm.Expr("flight_number - ?", 1)).Error
	if err != nil {
		return err
	}
	var planetsrequest2 models.PlanetsRequest
